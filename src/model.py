import pandas as pd
import numpy as np
from src.metrics import matthews_correlation_coefficient, false_positives
...
class base_detector():
    ...
    def score(self, X, y):
        """Computes the Matthews Correlation Coefficient (MCC) of the model

        Parameters
        ----------
            X : array, sample features to use
            y : array, labels corresponding to the sample features
        
        Returns
        -------
            score : int, the MCC of the model (0 is random guessing, 1 is perfect correlation)
        """
        return matthews_correlation_coefficient(y, self.predict(X))
    ...
    def false_positives(self, X, y):
        ...
        Returns
        -------
            score : int, the number of false positives (ideally zero)
        """
